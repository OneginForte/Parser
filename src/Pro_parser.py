...
class Parser:
    ...
    def parse_grp(self, data_byte):
        ...
        # data_byte[29]+40 полное имя группы 
        # data_byte[28] число байт в полном имени группы 

        # Выделяем data_byte[0] байт имени группы из записи в 15 байт
        ...
        # Номер группы к оторой относится как результирующей
        W = data_byte[27] 
        tf.append(W)
    ...
    def parse_pro(self, grp_d, data_byte, group_rule, view_rule, time_rule):
        ...
        if data_byte[0] == 0:
            ...
            # Выделяем data_byte[10] байт имени участника из записи в 24 байт.
